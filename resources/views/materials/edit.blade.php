...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ route('materials.update', $material) }}" class="bg-white rounded-lg shadow p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="sku" value="{{ old('sku', $material->sku) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $material->name) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $material->description) }}</textarea>
        ...
        <div>
            ...
            <select name="unit_of_measure" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                ...
                <option value="kg" {{ old('unit_of_measure', $material->unit_of_measure) == 'kg' ? 'selected' : '' }}>Kilogram (kg)</option>
                <option value="meter" {{ old('unit_of_measure', $material->unit_of_measure) == 'meter' ? 'selected' : '' }}>Meter (m)</option>
                <option value="liter" {{ old('unit_of_measure', $material->unit_of_measure) == 'liter' ? 'selected' : '' }}>Liter (L)</option>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-gray-700 font-medium mb-2">Current Stock *</label>
                <input type="number" name="quantity_in_stock" value="{{ old('quantity_in_stock', $material->quantity_in_stock) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" min="0" required>
                <p class="text-xs text-gray-600 mt-1">Use Add/Remove Stock for adjustments</p>
            ...
            <div>
                ...
                <input type="number" name="reorder_level" value="{{ old('reorder_level', $material->reorder_level) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" min="0" required>
        ...
        <div>
            ...
            <input type="number" name="unit_price" step="0.01" value="{{ old('unit_price', $material->unit_price) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
            ...
            <div class="space-y-2">
                @foreach ($suppliers as $supplier)
                    <label class="flex items-center">
                        <input type="checkbox" name="suppliers[]" value="{{ $supplier->id }}" {{ in_array($supplier->id, old('suppliers', $selectedSuppliers)) ? 'checked' : '' }} class="rounded border-gray-300">
        ...
        <div>
            ...
            <textarea name="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('notes', $material->notes) }}</textarea>
